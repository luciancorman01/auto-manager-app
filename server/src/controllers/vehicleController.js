...
const createVehicle = async (req, res) => {
  ...
  if (!VIN_REGEX.test(vinTrimmed)) {
    return res.status(400).json({ message: "VIN-ul trebuie să aibă exact 17 caractere alfanumerice valide." });
  }

  const nrTrimmed = String(nr_inmatriculare).trim().toUpperCase();
  if (!NR_INMATRICULARE_REGEX.test(nrTrimmed)) {
    return res.status(400).json({ message: "Numărul de înmatriculare nu este valid (ex: BH 01 ABC)." });
  }

  const an = Number(an_fabricatie);
  if (!Number.isInteger(an) || an < 1900 || an > CURRENT_YEAR) {
    return res.status(400).json({ message: `Anul de fabricație trebuie să fie între 1900 și ${CURRENT_YEAR}.` });
  }

  if (String(marca).trim().length < 2 || String(marca).trim().length > 50) {
    return res.status(400).json({ message: "Marca trebuie să aibă între 2 și 50 de caractere." });
  }
  if (String(model).trim().length < 1 || String(model).trim().length > 50) {
    return res.status(400).json({ message: "Modelul trebuie să aibă între 1 și 50 de caractere." });
  }

  if (poza !== undefined && poza !== null && poza !== "") {
    if (typeof poza !== "string" || !poza.startsWith("data:image/")) {
      return res.status(400).json({ message: "Formatul imaginii nu este valid." });
    }
    if (poza.length > 2_800_000) {
      return res.status(400).json({ message: "Imaginea este prea mare. Maxim 2MB." });
    }
  }
...
const updateVehicle = async (req, res) => {
  ...
  try {
    ...
    if (nr_inmatriculare !== undefined) data.nr_inmatriculare = String(nr_inmatriculare).trim().toUpperCase();
